...
<style>
    ...
    .count-badge { display: inline-block; padding: 2px 10px; border-radius: 20px; background: #fefce8; color: #a16207; font-size: 0.75rem; font-weight: 700; }
...
<div class="toolbar">
    ...
    <div class="toolbar-right">
        <a href="/classrooms/export" class="btn btn-outline">
        ...
        <a href="/classrooms/create" class="btn btn-primary">
...
<div class="table-wrap">
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                <td style="color:#9ca3af;">{{ $classrooms->firstItem() + $i }}</td>
                <td style="font-weight:500;">{{ $classroom->name }}</td>
                <td>{{ $classroom->room_number ?? '—' }}</td>
                <td><span class="count-badge">{{ $classroom->enrollments_count }} ta</span></td>
                <td style="font-size:0.8rem;color:#9ca3af;">{{ $classroom->created_at->format('d.m.Y') }}</td>
                <td>
                    <div class="actions">
                        <a href="/classrooms/{{ $classroom->id }}/edit" class="btn btn-sm btn-warning">Tahrirlash</a>
                        <form method="POST" action="/classrooms/{{ $classroom->id }}" onsubmit="return confirm('O\'chirishni tasdiqlaysizmi?')">
                            @csrf @method('DELETE')
    ...
    @else
    <div class="empty">
        <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#d1d5db" stroke-width="1.5"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9h18"/><path d="M9 21V9"/></svg>
